...
class UrlBar extends HTMLElement {
	constructor() {
			...
			if(e.keyCode === 13) {
				...
				validate.url(data, (resp) => {
					if(resp.valid) {
						...
						let url = "https://www.baidu.com/s?wd=" + resp.url
						this.dispatchEvent(new CustomEvent('enter', { detail: url}));
					}
				});
			}
		});
		...
		URlMenu.append(new MenuItem({label: '复制标题', click: () => { tabs.activePage.getTitle(Pt => clipboard.writeText(Pt)); }}));
		URlMenu.append(new MenuItem({label: '复制链接', click: () => { tabs.activePage.getURL(Purl => clipboard.writeText(Purl)); }}));
		URlMenu.append(new MenuItem({label: '粘贴', click: () => { this.value = clipboard.readText(); }}));
